...
export default function Eyes({ activeApp = "about" }: { activeApp?: AppKey }) {
      ...
      try {
        ...
        const sketch = (p: any) => {
          ...
          p.draw = () => {
            ...
            const eyeSpacing = isMobile ? 80 : 120;
            const cx1 = p.width / 2 - eyeSpacing / 2;
            const cy1 = isMobile ? 80 : 100;
            const cx2 = p.width / 2 + eyeSpacing / 2;
            const cy2 = isMobile ? 80 : 100;

            const eyeRx = isMobile ? 35 : 50;
            const eyeRy = isMobile ? 30 : 42;
            const pupilR = isMobile ? 8 : 12;
            ...
            const drawPupil = (cx: number, cy: number) => {
              ...
              // Limit pupil movement within eye bounds (responsive)
              const maxMoveX = eyeRx * (isMobile ? 0.5 : 0.6);
              const maxMoveY = eyeRy * (isMobile ? 0.5 : 0.6);

              // Calculate pupil position with following (responsive sensitivity)
              const sensitivity = isMobile ? 0.2 : 0.3;
              const pupilX =
                cx +
                Math.max(
                  -maxMoveX,
                  Math.min(maxMoveX, dx * followStrength * sensitivity)
                );
              const pupilY =
                cy +
                Math.max(
                  -maxMoveY,
                  Math.min(maxMoveY, dy * followStrength * sensitivity)
                );
        ...
        if (containerRef.current) {
          ...
          const updateFallbackEyes = (e: MouseEvent) => {
            ...
            eyes.forEach((eye) => {
              ...
              const maxMove = 25;
              const moveX = Math.max(-maxMove, Math.min(maxMove, dx * 0.4));
              const moveY = Math.max(-maxMove, Math.min(maxMove, dy * 0.4));
